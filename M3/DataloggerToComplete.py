...
try:
    ...
    for i in times:
        ...
        time.sleep(0.25)
        voltage_reading = float (meter.query('MEAS:VOLT:DC?'))
        # room temp = 25 -> the room voltage is 2.98 because of the Vout(T0)
        temp_reading = float( 298 * (voltage_reading)/2.980 - 273.15 + 25)
        # print(temp_reading, i)
        temperature[int(i) - 1] = (temp_reading)
